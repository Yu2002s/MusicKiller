 ...
 * 对音乐播放封装的基类
 */
abstract class BaseMusicFragment<V : ViewDataBinding>(@LayoutRes contentLayoutId: Int = 0) :
   ...
   * 下载服务实例
   */
  val downloadService get() = activity._downloadService
  ...
  /**
   * 是否开启 MediaController，启用后可控制播放
   */
  var enableMediaController: Boolean = true
   ...
   */
  abstract fun onCreateMediaController(controller: MediaController)
  ...
  protected open fun getMusicModels(): List<Any?>? {
    return getMusicRecyclerView()?.bindingAdapter?.models
  }

  /**
   * 获取音乐RecyclerView
   *
   * @return 如果不返回 null，则表示将代理此RecyclerView 创建默认的音乐视图
   */
  protected open fun getMusicRecyclerView(): RecyclerView? {
    return null
  }
    ...
    if (enableMediaController) {
      viewLifecycleOwner.lifecycleScope.launch {
        mainViewModel.musicModel.collect {
          this@BaseMusicFragment.onMusicModelChanged(it)
        }
      }

      registerMusicBroadReceiver()
    }
  }

  /**
   * 注册 MusicBroadReceiver
   */
  private fun registerMusicBroadReceiver() {
   ...
   * 添加本地音乐到播放队列
   *
   * @param downloadModel 下载对象
   * @param immediate 是否立即播放
   * @param onError 播放失败回调
   */
  protected fun addLocalPlay(
   ...
   */
  fun addPlaylist(list: List<Any?>, clear: Boolean = true, play: Boolean = true) {
    _mediaController ?: return
   ...
   * 添加到下载队列
   */
  fun addDownload(model: MusicModel = musicModel) {
    ...
    val hasLossless = model.hasLossless
    ...
    MaterialAlertDialogBuilder(requireContext())
      ...
      .setPositiveButton("下载") { _, _ ->
        ...
        downloadService?.addDownload(model, currentBridge)
      }
   ...
   * 设置当前播放音乐的音质
   */
  fun setMusicBridge(bridge: MusicBridge? = null) {
    // 修改音质
    val qualities = MusicBridge.entries.map { it.level }.toTypedArray()
    var currentBridge = bridge ?: SPConfig.CURRENT_BRIDGE.getRequired(MusicBridge.MP3_128K.level)
   ...
   * 添加或收藏音乐
   */
  fun addOrRemoveFavorite(model: MusicModel = musicModel) {
   ...
   */
  fun playOrPause(v: View? = null): Boolean {
    // 播放状态修改
